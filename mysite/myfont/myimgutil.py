...
def catImg(img_path_list,img_save_path, img_num, img_size=360):
    img_row=1+int(img_num/10)
    img_col=10
    ...
    result = Image.new(_img_list[0].mode, (img_col*img_size, img_row*img_size),color="white")
    cnt=0
...
class OneImgProcess:
    ...
    LIMIT_LINE = 30
    cubeSize=500
    CachePicSavePathBase=r""
    timeStart=time.strftime(r'%y-%b-%d %H:%M:%S',time.localtime(time.time()))+"_"
    ...
    def getRidOfPageCube(self,pic):
        ...
        savePath=self.CachePicSavePathBase+self.timeStart+r'getRidOfPageCube.jpg'
    ...
    def imgBinary(self,pic_copy):
        ...
        # imgBinary_ok = 1
        savepath=self.CachePicSavePathBase+self.timeStart+r'Binery.jpg'
    ...
    def imgTransform(self,pic, points):
        ...
        savepath=self.CachePicSavePathBase+self.timeStart+r'imgTransform.jpg'
    ...
    def imgRotate(self,pic, direction):
        ...
        if direction == 1:  # 正向，无须矫正
            ...
            print('图像方向判断出错')
            return 0
        savepath=self.CachePicSavePathBase+self.timeStart+r'imgRotate.jpg'
    ...
    def cutImage(self,pic, size):
        ...
        savepath=self.CachePicSavePathBase+self.timeStart+r'cutImage.jpg'
    ...
    def drawProjection(self,projectLs,fullPix,dir):
        if dir=="Horizontal":
            ...
            self.imgSave(self.CachePicSavePathBase+self.timeStart+r"Horizontal.jpg",img)
            ...
            for colIdx in range(0,sz):
                for rowIdx in range(0,projectLs[colIdx]):
                    img[rowIdx][colIdx]=self.BLACK
            self.imgSave(self.CachePicSavePathBase+self.timeStart+r"vertical.jpg",img)
        else:
            print("投影方向出错")
            return
...
# 涂白表格线并返回
    def clearLine(self,pic): #传入一个图，返回一个去除所有，只剩汉字的图片
        ...
        self.imgSave(self.CachePicSavePathBase+self.timeStart+r"WhiteLine.jpg",pic)
    ...
    def cutSubImg_Save(self,savePath,page,page_level,noLinePic,HrztCrestList,vtclCrestList):
        ...
        for rowBotIdx in range(1,len(HrztCrestList)):
            ...
            for colBotIdx in range(1,len(vtclCrestList)):
                ...
                self.img_names.append(str(wordId)+'_'+str(page)+'_'+str(rowBotIdx)+'_'+str(colBotIdx)+".jpg")
