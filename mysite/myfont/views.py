        ...
        with open(file_path, "rb") as file:
            while True:
                ...
                if chuck_stream:
                    ...
                    break
    file_path = 'C:\\Users\\chasu\\Desktop\\myfontPRO\\mysite\\myfont\\static\\myfont\\assets\\fontTable\\GB2312_1.pdf'
...
def activity(request):
    ...
    if request.method == 'POST':
        ...
        catImg(img_path_list=img_path_list, img_save_path=img_save_path,img_num=len(article) )
...
def createfont(request):
    ...
    if request.method == 'POST':
        ...
        for inputfile in inputfiles:    # 依次处理各个文件
            ...
            destination.close()
        img_process_names = imgpreprocess(useruploadpath, userprocesspath, index, inputfiles)
        img2outline(userprocesspath, useroutlinepath, index, img_process_names)
        outline2bezier(useroutlinepath, index, user_id, img_process_names)
...
def imgpreprocess(useruploadpath, userprocesspath, index, inputfiles):
    '''
    ...
    img_list = []
    for img_name in inputfiles:
        img_read_path = readpath + '\\' + img_name.name #该变量就是要读取的图片的完整路径 'D:\\user_id\\upload\\1.jpg'
        # 进行后续处理
        a = OneImgProcess(img_read_path, writepath)
        img_list.extend(a.img_names)
...
def img2outline(userprocesspath, useroutlinepath, index, img_process_names):
    '''
    ...
    # img_names = read_images(readpath)
    for img_name in img_process_names:
...
def outline2bezier(useroutlinepath, index, user_id, img_process_names):
    '''
    ...
    cnt = 0
    for img_name in img_process_names:
        character_index = img_name.split('_')[0]
        ...
        if not Character.objects.filter(index=character_index).exists():
            return
        user = User.objects.get(id=user_id)
        character = Character.objects.get(index=character_index)
...
def addCharacter(request):
    with open(r'C:\Users\chasu\Desktop\myfontPRO\save_1_3755.txt', 'r') as f:
        str = f.read()
